...
class DataTicket(models.Model):
    """Tickets submitted by offices for new training data"""
    STATUS_CHOICES = [
    ...
    # Ticket Information
    ticket_number = models.CharField(max_length=40, unique=True, editable=False)  # increased a bit for future-proofing
    ...
    new_intent_name = models.CharField(max_length=150, blank=True, help_text="If creating new intent")  # increased
    ...
    # Link to created training data
    training_data = models.ForeignKey(TrainingData, on_delete=models.SET_NULL, null=True, blank=True, related_name='source_ticket')
    
    class Meta:
        ordering = ['-created_at']
    
    def __str__(self):
        return f"{self.ticket_number} - {self.office.name} - {self.status}"
    
    def save(self, *args, **kwargs):
        if not self.ticket_number:
            # Generate ticket number: TICK-YYYYMMDD-XXX
            today = timezone.now().strftime('%Y%m%d')
            count = DataTicket.objects.filter(ticket_number__startswith=f'TICK-{today}').count()
            self.ticket_number = f'TICK-{today}-{count+1:03d}'
        super().save(*args, **kwargs)

class TicketComment(models.Model):
    """Comments/discussion on tickets"""
    ticket = models.ForeignKey(DataTicket, on_delete=models.CASCADE, related_name='comments')
...
class LocationTicket(models.Model):
    """Tickets submitted by offices for new locations/rooms"""
    ...
    submitted_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='submitted_location_tickets')
    office = models.ForeignKey(Office, on_delete=models.CASCADE, related_name='location_tickets')
    
    # Location Content
    room_number = models.CharField(max_length=200, help_text="e.g., 401, Smart Lab")  # increased
    room_name = models.CharField(max_length=255, blank=True, help_text="Descriptive name (optional)")
    building = models.CharField(max_length=200, help_text="e.g., Building A")
    floor = models.CharField(max_length=50, help_text="e.g., 4th Floor")
    ...
    notes = models.TextField(blank=True, help_text="Additional context or notes")
    admin_notes = models.TextField(blank=True, help_text="Admin feedback")
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviewed_location_tickets')
    reviewed_at = models.DateTimeField(null=True, blank=True)
    ...
    # Link to created location
    location = models.ForeignKey(Location, on_delete=models.SET_NULL, null=True, blank=True, related_name='source_ticket')
    ...
    def __str__(self):
        return f"{self.ticket_number} - {self.room_number} ({self.building}) - {self.status}"
    
    def save(self, *args, **kwargs):
        if not self.ticket_number:
            # Generate ticket number: LOC-YYYYMMDD-XXX
            today = timezone.now().strftime('%Y%m%d')
            count = LocationTicket.objects.filter(
                ticket_number__startswith=f'LOC-{today}'
            ).count()
            self.ticket_number = f'LOC-{today}-{count+1:03d}'
        super().save(*args, **kwargs)


class LocationTicketComment(models.Model):
    """Comments/discussion on location tickets"""
    ticket = models.ForeignKey(LocationTicket, on_delete=models.CASCADE, related_name='comments')
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    comment = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
