...
class ChatbotEvaluator:
    ...
    def export_graphs(self, results: dict, out_dir: str = "evaluation_output"):
        ...
        fig, ax = plt.subplots(figsize=(14, 12))  # increased size

        cm_data = np.array(results["confusion_matrix"])
        labels  = results["intent_labels"]

        # Shorten labels (cleaner display)
        short = [
            l.replace("_", " ")[:15] + "..." if len(l) > 15 else l.replace("_", " ")
            for l in labels
        ]

        # Remove zero annotations
        annot = np.where(cm_data == 0, "", cm_data)
